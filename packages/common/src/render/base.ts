...
export class BaseRenderer implements Renderer {
  ...
  _initPipeline(options: BaseRendererOptions) {
    ...
    function getShaderSource(shader?: ShaderCode): string {
      if (!shader) {
        return ShaderCode.default().source;
      }
      // console.log('source', shader.source);
      return shader.source;
    }

    try {
      // console.log(getShaderSource(options.vertexShader));
      const vertexShaderModule = this.device.createShaderModule({
        code: getShaderSource(options.vertexShader),
      });
      vertexShaderModule.getCompilationInfo().then((info) => {
        // TODO, show compilation info data to users for debugging.
      ...
      console.log(this.options.vertexShader);
      console.log('error compiling vertex', e);
    }
  }
